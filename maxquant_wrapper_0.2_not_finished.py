...
def without_params(file_locations, database_file, template_file, folder):
   ...
   fraction_tag = template_soup.fractions
   fraction_tag.clear()

   group_tag = template_soup.paramGroupIndices
   group_tag.clear()
   ...
   for i in file_locations: 
      ...
      new_fraction_tag = template_soup.new_tag("short")
      fraction_tag.append(new_fraction_tag)
      new_fraction_tag.string = r"1"
      new_group_tag = template_soup.new_tag("int")
      group_tag.append(new_group_tag)
      new_group_tag.string = r"0"
   ...
   cmd7 = r"copy " + str(folder) + r"combined\txt\proteinGroups.txt ." 
   os.system(cmd7)

   os.rename("proteinGroups.txt", str(sys.argv[7]))
   #Renames the proteinGroups.txt what is specified as the output by Galaxy
...
def has_params(file_locations_joined, database_file, template_file, folder):
   ...
   #rename_folder = str(folder_build) + time.strftime("_%d_%m_%Y_%H_%M") + "_combined"
   #cmd9 = r"mkdir J:\Proteomics\mq_jobs\combined\\" + str(rename_folder)
   #os.system(cmd9)
   #cmd10 = r"Xcopy /E /I " + str(folder) + r"combined J:\Proteomics\mq_jobs\\" + str(rename_folder)
   #os.system(cmd10)
   #Currently this copy does not work properly but may be a permissions issue

   cmd7 = r"copy " + str(folder) + r"combined\txt\peptides.txt ."
   os.system(cmd7)

   os.rename(r"peptides.txt", str(sys.argv[6]))
   #Renames the summary.txt what is specified as the output by Galaxy

   cmd8 = r"copy " + str(folder) + r"combined\txt\proteinGroups.txt ." 
   os.system(cmd8)

   os.rename(r"proteinGroups.txt", str(sys.argv[7]))
   #Renames the proteinGroups.txt what is specified as the output by Galaxy
...
if (has_xml == "true"):
   ...
   if (exp_design == "4plex iTRAQ-phosphoSTY"):
      template_file = open(r'C:\mq_jobs\param_files\4plex_iTRAQ-phosphoSTY_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder)
   elif (exp_design == "4plex iTRAQ-Protein-ID"):
      template_file = open(r'C:\mq_jobs\param_files\4plex_iTRAQ-Protein-ID_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder)
   elif (exp_design == "6plex-TMT-Protein-ID"):
      template_file = open(r'C:\mq_jobs\param_files\6plex-TMT-Protein-ID_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder) 
   elif (exp_design == "6plex-TMT-pSTY"):
      template_file = open(r'C:\mq_jobs\param_files\6plex-TMT-pSTY_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder) 
   elif (exp_design == "Label-Free-ABPP"):
      template_file = open(r'C:\mq_jobs\param_files\Label-Free-ABPP_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder) 
   elif (exp_design == "Label-Free-phosphoSTY"):
      template_file = open(r'C:\mq_jobs\param_files\Label-Free-phosphoSTY_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder)    
   elif (exp_design == "Label-Free-Protein-ID"):
      template_file = open(r'C:\mq_jobs\param_files\Label-Free-Protein-ID_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder) 
   elif (exp_design == "Label-Free-PRTC"):
      template_file = open(r'C:\mq_jobs\param_files\Label-Free-PRTC_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder) 
   elif (exp_design == "SILAC-K6R10-phosphoSTY"):
      template_file = open(r'C:\mq_jobs\param_files\SILAC-K6R10-phosphoSTY_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder) 
   elif (exp_design == "SILAC-K6R10-Protein-ID"):
      template_file = open(r'C:\mq_jobs\param_files\SILAC-K6R10-Protein-ID_mqpar.xml','r')
      #Selects mqpar.xml file
      has_params(file_locations, database_file, template_file, working_folder)
   elif (exp_design == "None"):
